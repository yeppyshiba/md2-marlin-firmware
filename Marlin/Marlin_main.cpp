...
// For M0/M1, this flag may be cleared (by M108) to exit the wait-for-user loop
  ...
  volatile bool wait_for_user=false; // = false;
#endif
...
inline void gcode_M17() {
  ...
  static bool load_filament(const float &slow_load_length=0, const float &fast_load_length=0, const float &purge_length=0, const int8_t max_beep_count=0,
    ...
    if (pause_for_user) {
      ...
      KEEPALIVE_STATE(PAUSED_FOR_USER);
      wait_for_user = true;    //jjj LCD click or M108 will clear this
      while (wait_for_user) {
        #if HAS_BUZZER
  ...
  static void wait_for_filament_reload(const int8_t max_beep_count=0) {
    ...
    KEEPALIVE_STATE(PAUSED_FOR_USER);
    wait_for_user = true;    // LCD click or M108 will clear this /jjj
    while (wait_for_user) {
      #if HAS_BUZZER
        filament_change_beep(max_beep_count);
      #endif
      ...
      if (nozzle_timed_out) {
        ...
        while (wait_for_user) idle(true); /jjj
